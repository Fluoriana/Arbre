...
displayHeader("Création d'un devis");
		...
		if (!is_ok_form_creation1()){
			...
			if (!isset($_POST['is_submit_form2'])){
				...
				if (!is_ok_form_creation2()){
					...
					$rabais_devis=trim($_POST['rabais']);
					$type_devis=trim($_POST['type_devis']);
					...
					$req_devis=pg_query($db,"SELECT id_devis FROM Devis ORDER BY id_devis DESC;");
					...
					$nb_devis= pg_fetch_row($req_devis);

					$id_client=$nb_client[0] + 1;
					$id_bien=$nb_bien[0] + 1;
					$id_devis=$nb_devis[0] + 1; 

					$montant=($prix_carre*$surface)*(1+($tva/100))-$rabais_devis;
					$annee=date("Y");
					$mois=date("m");
					$jour=date("d");
					...
					if ($is_already_client==0){
						...
						if ($is_already_bien==0){
							...
							$res_devis= pg_query($db,"INSERT INTO Devis VALUES ($id_devis, $id_client, $id_bien, $montant, 
																$tva, '$date', $prix_carre, $rabais_devis,$type_devis);");
						}
						else{
							...
							$res_devis= pg_query($db,"INSERT INTO Devis VALUES ($id_devis, $id_client, 
													  $tab_bien[0], $montant, $tva, '$date', $prix_carre, $rabais_devis,$type_devis);");
						}
					}
					...
					if ($is_already_client==1){
						...
						if ($is_already_bien==0){
							...
							$res_devis= pg_query($db,"INSERT INTO Devis VALUES ($id_devis, $tab_client[0], 
															  $id_bien, $montant, $tva, '$date', $prix_carre, $rabais_devis, $type_devis);");
						}
						else{
