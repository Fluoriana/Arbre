...
if (isset($_SESSION['admin'])){
	...
	if (!isset($_POST['soumission1'])){
		...
		$req = pg_query($db, "SELECT id_bien_facture, id_client_facture FROM Facture WHERE id_facture=$id_facture;");
		...
		$res3=pg_query($db,"SELECT * FROM Facture WHERE id_facture=$id_facture;");
		...
		if (!isset($_POST['soumission2'])){
			print("<form action=\"modif_facture.php\" method=\"post\">");
			print("<p>Nom / Nom de Société : <input type=\"hidden\" name=\"nom\" value=\"$tab1[1]\"/> $tab1[1]</p>
		   		<p>Civilite : <input type=\"hidden\" name=\"civilite\" value=\"$tab1[3] /> $tab
		   		<p>Adresse du client : <input type=\"hidden\" name=\"adresse_client\" value=\"$tab1[4]\" /> $tab1[4]</p>
			...
			if (is_ok_form_modif()){
				...
				$tva=$_POST['tva'];
				$rabais=$_POST['rabais'];
				$type_facture=$_POST['devis'];

				pg_query($db,"UPDATE Bien SET adresse_bien='$adresse_bien', cp_bien=$cp_bien, ville_bien='$ville_bien',
			                         surface=$surface, surface_terrain=$surface_terrain, type_bien=$type_bien, nb_pieces=$nb_piece
			    	                 WHERE id_bien=$id_bien;");

				pg_query($db,"UPDATE Facture SET prix_carre_devis=$prix_carre, tva=$tva, rabais_facture=$rabais, type_facture=$type_facture
				 					 WHERE id_facture=$id_facture;");
			}
			else{
				print("<form action=\"modif_facture.php\" method=\"post\">");
				print("<p>Nom / Nom de Société : <input type=\"hidden\" name=\"nom\" value=\"$tab1[1]\"/> $tab1[1]</p>
					   <p>Civilite : <input type\"hidden\" name=\"civilite value=\"$tab1[3] </p>
	   			   	   <p>Adresse du client : <input type=\"hidden\" name=\"adresse_client\" value=\"$tab1[4]\"/> $tab1[4]</p>
       			   	   <p>CODE POSTAL : <input type=\"hidden\" name=\"cp_client\" value=\"$tab1[5]\"/> $tab1[5]</p>
	   			       <p>Ville : <input type=\"hidden\" name=\"ville_client\" value=\"$tab1[6]\"/> $tab1[6]</p>
	   			       <p>Adresse électronique : <input type=\"hidden\" name=\"mail\" value=\"$tab1[7]\"/> $tab1[7]</p>
